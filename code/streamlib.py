...
#STREAMFILE = os.path.join(DATADIR,'streams_v5.1.yaml')
...
class DESSkymapNGC288(DESSkymapMcBryde):
...
class DESSkymapNGC1261(DESSkymapMcBryde):
    """Class for plotting a zoom on DES. This is relatively inflexible."""
    # RA, DEC frame limits
    FRAME = [[47,48],[-60,-50]]
    FIGSIZE = [3.4,5]
...
class DESSkymapNGC1851(DESSkymapMcBryde):
    """Class for plotting a zoom on DES. This is relatively inflexible."""
    # RA, DEC frame limits
    #FRAME = [[80,80],[-50,-30]]
    FRAME = [[80,80],[-47,-33]]
...
class DESSkymapNGC1904(DESSkymapMcBryde):
    """Class for plotting a zoom on DES. This is relatively inflexible."""
    # RA, DEC frame limits
...
class DECaLSSkymap():
    """Class for plotting a zoom on DES. This is relatively inflexible."""
    # RA, DEC frame limits
    FRAME = [[-180,-180,180,180],[30,-30,30,-30]]
    FIGSIZE=(8,5)

    def __init__(self, *args, **kwargs):
        defaults = dict(lon_0=0,celestial=False, projection='ortho')
        setdefaults(kwargs,defaults)
...
def skymap_factory(proj):
    ...
    if proj == 'q1':
        ...
        smap = skymap.survey.DESSkymapQ2_zoom
    elif proj == 'q3':
        ...
        smap = skymap.survey.DECaLSSkymapPal13
    elif proj == 'eriphe':
        smap = skymap.survey.DESSkymapEriPhe
    else:
...
def draw_arc(smap,points,coord='cel',offset=0,**kwargs):
    ...
    if coord == 'gal': lons,lats = cel2gal(lons,lats)
...
def inv_rotation(phi1, phi2, R):
    ra, dec = rotation_matrix.phi12_rotmat(phi1, phi2, np.linalg.inv(R))
...
def draw_globulars(smap,mod,coord='cel'):
    ...
    for name,values in GLOBULARS.items():
        ...
        if coord == 'gal': lon,lat = cel2gal(lon,lat)
...
def draw_grillmair17(smap,coord='cel',offset=1.0,**kwargs):
    ...
    for stream,c,dec in [molonglo,murrumbidgee]:
        ...
        ra = poly(dec)
        if coord == 'gal': lon,lat = cel2gal(ra,dec)
        else: lon,lat = ra,dec
...
def draw_agnello17(smap, coord='cel', offset=0.0, **kwargs):
    # Interpolate in ra from dec
    wg1 = dict(name='WG1',ends=[[64.16,-23.2],[97.2,-55.5]])
    wg2 = dict(name='WG2',ends=[[70.5,-51.4],[95.7,-47.1]])
    wg3 = dict(name='WG3',ends=[[304.1,-47.7],[351.3,-44.8]])
    wg4 = dict(name='WG4',ends=[[300.0,-49.4],[333.5,-66.0]])
    
    for stream in [wg1,wg2,wg3,wg4]:
        draw_stream_arc(smap,stream,coord=coord,offset=offset,**kwargs)

    for stream in [wg1]:
        #14/np.cos(np.radians(e[1]))

        #stream['ends'] = [[e[0]+6/np.cos(np.radians(e[1])),e[1]] for e in stream['ends']]
        ends = [[e[0]+6/np.cos(np.radians(e[1])),e[1]] for e in stream['ends']]
        #ends = [[e[0]+8,e[1]] for e in stream['ends']]
        stream['ends'] = ends
