...
def chi2_worker(mu_phi1cosphi2_prog, mu_phi2_prog, rv_prog, dist_prog, phi2_prog, sigma_track, sigma_vr, mu_alpha_lmc, mu_delta_lmc, rv_lmc, dist_lmc, M_LMC, rs_LMC, tmax, pid, lmc_disk):
    ...
    if ORPHAN:
        s5_data = np.loadtxt('../Chenab_Orphan_S5.txt')
        print('Using full Orphan data!')
    ...
    # phi1_rrls, phi2_rrls = phi12_rotmat(orph_rrls['ra'], orph_rrls['dec'], R_phi12_radec)
    # phi2_rrls = phi2_rrls[np.abs(phi1_rrls) < stream_lengths[STREAM]/2.]
    # phi1_rrls = phi1_rrls[np.abs(phi1_rrls) < stream_lengths[STREAM]/2.]

    # for counter in range(len(phi1_rrls)):
    #     t_var = phi1_rrls[counter]
    ...
    #                 return np.sum(-np.log(np.sqrt(2 * np.pi) * truncnorm.pdf(phi2_data, a, b, loc, scale)))
    ...
    for counter in range(len(phi1_s5)):
        ...
        if len(mask) > 5:
            ...
            # TEST
            # print('phi1, b_error = ', t_var, b_error)
            # b_phi1s.append(t_var)
    ...
    if DIST == True:
        ...
        if ORPHAN:
            print('Including Orphan RRLs!')
            dist_data = dist_data[(dist_data['stream'] == 'Chenab') | (dist_data['stream'] == 'Orphan')]
        else:
