...
R_phi12_radec = np.array([[-0.69798645,  0.61127501, -0.37303856],
                          [-0.62615889, -0.26819784,  0.73211677],
                          [0.34747655,  0.744589,  0.56995374]])  # from Peter
M_prog = 2.e-6  # stream_masses[STREAM]  # 2.e-6 # 1.e-3
sigma_prog = 0.01  # stream_sigmas[STREAM]
...
def chi2_worker(mu_phi1cosphi2_prog, mu_phi2_prog, rv_prog, dist_prog, phi2_prog, sigma_track, sigma_vr, mu_alpha_lmc, mu_delta_lmc, rv_lmc, dist_lmc, M_LMC, rs_LMC, tmax, pid):
    ...
    phi1_jetdata, phi2_jetdata, e_phi2_jetdata = np.loadtxt('../jet/jet_track.txt', unpack=True)
    ...
    for counter in range(len(phi1_jetdata)):
        t_var = phi1_jetdata[counter]
        ...
        mu_obs = phi2_jetdata[counter]
        sigma_obs = e_phi2_jetdata[counter]
    ...
    # pm_data = np.genfromtxt('../jet/pm_result_v1.2.1.csv', names=True, delimiter=',')
    # pm1r_stream, pm2r_stream = pmphi12_reflex(alpha, delta, pmra, pmdec, R_phi12_radec, r_stream)

    chi2_pmra = 0.

    # for counter in range(len(pm_data)):
    for counter in range(1):
        # t_var = (pm_data[counter]['phi1_min'] + pm_data[counter]['phi1_max']) / 2.
        t_var = 0.
        # mask = np.where((phi1 > pm_data[counter]['phi1_min']) * (phi1 < pm_data[counter]['phi1_max']))[0]  # approx length fit with gmm
        mask = np.where(np.abs(phi1) < 13.)
        if len(mask) > 5:

            # COMPUTE MAX LIKELIHOOD LOCATION OF STREAM OBSERVABLE
            # phi1_data = phi1[mask]
            phi1_data = phi1[mask] / 10.  # to match peter's fit
            # phi2_data = pm1r_stream[mask]  # This needs to change for each observable  #########
        ...
        # COMPARE SIMULATION TO DATA
        # mu_obs = pm_data['mean_mu_phi1'][counter]
        # sigma_obs = pm_data['std_mu_phi1'][counter]
        # sigma = (sigma_data**2. + sigma_obs**2.)**0.5
        ...
        mu_data = b_estimate
        sigma_data = b_error
        mu_obs = -1.546680  # from Peter
        sigma_obs = 0.021048  # from Peter
        sigma = (sigma_data**2. + sigma_obs**2.)**0.5
        chi2_pmra += np.log(np.sqrt(2. * np.pi * sigma**2.)) + 0.5 * (mu_data - mu_obs)**2. / (sigma)**2.
    ...
    for counter in range(1):
        ...
        if len(mask) > 5:
            ...
            # COMPUTE MAX LIKELIHOOD LOCATION OF STREAM OBSERVABLE
            # phi1_data = phi1[mask]
            phi1_data = phi1[mask] / 10.
            # phi2_data = pm2r_stream[mask]  # This needs to change for each observable  #########
        ...
        # COMPARE SIMULATION TO DATA
        # mu_obs = pm_data['mean_mu_phi2'][counter]
        # sigma_obs = pm_data['std_mu_phi2'][counter]
        # sigma = (sigma_data**2. + sigma_obs**2.)**0.5
        ...
        mu_data = b_estimate
        sigma_data = b_error
        mu_obs = -0.430494  # from Peter
        sigma_obs = 0.018994  # from Peter
        sigma = (sigma_data**2. + sigma_obs**2.)**0.5
        chi2_pmdec += np.log(np.sqrt(2. * np.pi * sigma**2.)) + 0.5 * (mu_data - mu_obs)**2. / (sigma)**2.
    ...
    # for counter in range(len(phi1_data)):
    #     t_var = phi1_data[counter]
    #     mask = np.where((phi1 > t_var - dL0 / 2.) * (phi1 < t_var + dL0 / 2.))[0]
    #     if len(mask) > 5:
    ...
    # DISTANCE #
    ############
    # if DIST == True and STREAM == 'Elqui':
    #     bhb_data = np.loadtxt('../elqui_dm.txt', skiprows=1)
    #     bhb_phi1 = bhb_data[:, 2]
    #     bhb_mod = bhb_data[:, 7]

    #     chi2_dist = 0.
    #     for counter in range(len(bhb_phi1)):
    #         t_var = bhb_phi1[counter]
    #         mask = np.where((phi1 > t_var - dL0 / 2.) * (phi1 < t_var + dL0 / 2.))[0]
    #         if len(mask) > 5:
    ...
    # elif DIST == True and STREAM == 'Chenab':
    #     print('Calculating chi2_dist...')
    #     rrl_data = fitsio.open('../orph.fits')[1].data[1:]
    #     rrl_phi1, rrl_phi2 = phi12_rotmat(rrl_data['ra'], rrl_data['dec'], R_phi12_radec)
    #     idx = np.abs(rrl_phi1) < stream_lengths[STREAM] / 2.
    #     rrl_data = rrl_data[idx]
    #     rrl_phi1 = rrl_phi1[idx]
    #     rrl_dist = rrl_data['heldist']
    ...
    if DIST == True:
        ...
        # bhb_data = fitsio.open('../stream_distance_bhb_v2.fits')[1].data
        # if STREAM == 'Jhelum':
        # bhb_data = bhb_data[bhb_data['stream'] == '%s%i' % (STREAM.replace('_', ' '), JHELUM_POP)]
        # elif STREAM == 'Tucana III':
        #     bhb_data = bhb_data[bhb_data['stream'] == 'Tuc3']
        # else:
        # bhb_data = bhb_data[bhb_data['stream'] == STREAM.replace('_', ' ')]

        # bhb_dist = mod2dist(bhb_data['dm'])
        # bhb_mod = bhb_data['dm']
        # bhb_phi1, bhb_phi2 = phi12_rotmat(bhb_data['ra'], bhb_data['dec'], R_phi12_radec)

        import pandas as pd
        dist_data = pd.read_csv('../jet/jet_distances.csv')
        dist_data = dist_data[dist_data['stream'] == STREAM]

        phi1_dist, phi2_dist = phi12_rotmat(dist_data['ra'].as_matrix(), dist_data['dec'].as_matrix(), R_phi12_radec)
        dm_dist = dist_data[['dm', 'dm_err']].as_matrix()
    ...
    if PLOTTING:
        ...
        ax[0].errorbar(phi1_data, phi2_data, yerr=np.sqrt(stream_widths[STREAM]**2 + sigma_track**2) * np.ones(len(phi1_data)),
        ...
        # ax[2].errorbar(phi1_data, vr_s5[:, 0], yerr=vr_s5[:, 1], c='r', marker='o',
        ...
        ax[2].errorbar(phi1_mu_obs, vr_mu_obs - vr_mu_data, yerr=vr_sig_obs, c='r', marker='o',
                       ecolor='r', markerfacecolor='r', capsize=3, linestyle='None', ms=1)
        # ax[2].errorbar(phi1_data, vr_s5[:, 0] - vr_mu_data, yerr=vr_s5[:, 1], c='r', marker='o',
        #                ecolor='r', markerfacecolor='r', capsize=3, linestyle='None', ms=1)
        ...
        ax[3].errorbar(phi1_data, pmra_s5[:, 0], yerr=pmra_s5[:, 1], c='r', marker='o',
                       ecolor='r', markerfacecolor='r', capsize=3, linestyle='None', ms=1)

        ax[4].errorbar(phi1_data, pmdec_s5[:, 0], yerr=pmdec_s5[:, 1], c='r', marker='o',
                       ecolor='r', markerfacecolor='r', capsize=3, linestyle='None', ms=1)
