...
def chi2_eval(mu_phi1cosphi2_prog, mu_phi2_prog, rv_prog, dist_prog, phi2_prog, M_LMC, sigma_track=0, sigma_vr=0, M_prog=2e-6, sigma_prog=0.01, mu_alpha_lmc=1.91, mu_delta_lmc=0.229, rv_lmc=262.2, dist_lmc=49970., lmc_disk=True):
    if lmc_disk == True:
        a_MN_LMC = 1.5
        b_MN_LMC = 0.3
        M_MN_LMC = 0.3
    ...
    lhood = chi2_worker(mu_phi1cosphi2_prog, mu_phi2_prog, rv_prog, dist_prog, phi2_prog, sigma_track, sigma_vr, M_prog, sigma_prog,
                        mu_alpha_lmc, mu_delta_lmc, rv_lmc, dist_lmc, M_LMC, rs_LMC, tmax, pid)
    return lhood


def chi2_worker(mu_phi1cosphi2_prog, mu_phi2_prog, rv_prog, dist_prog, phi2_prog, sigma_track, sigma_vr, M_prog, sigma_prog, mu_alpha_lmc, mu_delta_lmc, rv_lmc, dist_lmc, M_LMC, rs_LMC, tmax, pid):
    ...
    phi1_jet, width_jet, e_width_jet = np.loadtxt('../jet/jet_widths.txt', unpack=True)
    ...
    for counter in range(len(phi1_jet)):
        t_var = phi1_jet[counter]
        ...
        if len(mask) > 5:
            ...
            mu_data = sigma_estimate
            sigma_data = sigma_error  # b_error  # sigma_estimate  # because comparing to individual data points, not binned fit
        ...
        mu_obs = width_jet[counter]
        sigma_obs = e_width_jet[counter]
        sigma = (sigma_data**2. + sigma_obs**2.)**0.5
        chi2_w += np.log(np.sqrt(2. * np.pi * sigma**2.)) + 0.5 * (mu_data - mu_obs)**2. / (sigma)**2.

        phi1_widths_stream.append(t_var)
        widths_stream.append(mu_data)
        e_widths_stream.append(sigma_data)
    ...
    print(M_prog, sigma_prog, chi2, file=fileout)
    ...
    if PLOTTING:
        ...
        plt.errorbar(phi1_widths_stream, widths_stream, yerr=e_widths_stream,  c='navy')
        plt.errorbar(phi1_jet, width_jet, yerr=e_width_jet, c='r')

        plt.xlabel(r'$\phi1_1\ \mathrm{(deg)}$')
        plt.ylabel(r'$w\ \mathrm{(deg)}$')

        plt.tight_layout()

        plt.savefig('../plots/%s_width_%2f_%i.png' %(STREAM.replace(' ', '_'), phi1_prog, pid), bbox_inches='tight')
